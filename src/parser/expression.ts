...
export default class ExpressionParser extends NodeUtils {
	...
	parseExpressionBody(topLevel: boolean): N.Node {
	...
	parseToken(topLevel: boolean, body?: N.Node): N.Node {
	...
	parseMaybeUnary(topLevel?: boolean, body?: N.Node) {
	...
	parseConstLiteral(node: N.ConstLiteral): N.ConstLiteral {
		...
		node.name = this.state.type.label as any
		this.next()
	...
	parseArgsList<T extends N.Node>(
		end: TokenType,
		_getNode?: () => T,
	): T[] {
		const args: T[] = []
		const getNode = _getNode || (() => this.parseExpressionBody(false) as any)
