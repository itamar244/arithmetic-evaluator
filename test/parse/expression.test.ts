...
type Expectation = [string, Node]

const expressions: Expectation[] = [
...
test('should parse expressions and statements fine', (t) => {
	...
	const toCheck = expressions
		.map((item): Expectation => [item[0], n.Expression(item[1])])
		.concat(statements)
...
const throwables: Array<[string, number, string]> = [
